@extends('layouts.app')
@section('content')
    <form action="{{ route('products.update', $product->id) }}" method="post" id="submitForm" enctype="multipart/form-data">
        @method('put')
        @csrf
        <div class="row">
            <div class="col-md-4">
                <h4>{{ __('share.edit') }} {{ __('modules/product.product') }}</h4>
            </div>
        ...
        <div class="card mb-1">
            <div class="card-body">
                        ...
                        <div class="tab-pane fade{{ $loop->first ? ' show active' : '' }}" id="navs-top-{{ $appLanguage->id }}" role="tabpanel">
                            <div class="row">
                                ...
                                <x-form.textarea name="description[{{ $appLanguage->code }}]" required="true"
                                              ...
                                              value="{{ $product->getTranslation('description', $appLanguage->code) }}"
                ...
                <div class="row">
                    ...
                    <div class="col-6">
                        <img id="imagePreview" class="img-fluid img-thumbnail rounded-2" src="{{ $product->image }}" style="max-height: 200px">
                    </div>
                </div>
            </div>
        </div>
