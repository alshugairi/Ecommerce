...
abstract class Enum
{
    ...
    public function __construct(mixed $value = null)
    {
        ...
        if (!static::has($value)) {
            if (static::$unknownValuesFallbackToDefault) {
                ...
                throw new UnexpectedValueException(sprintf('Given value (%s %s) is not in enum `%s`', gettype($value), is_string($value) ? '"' . $value . '"' : $value, static::class));
            }
        }
    ...
    /**
     * Returns the consts (exceptServers for __DEFAULT) of the class.
     *
     * @return array
     */
    ...
    {
        return $this->equals(static::create(constant(static::class . '::' . $const)));
    }
    ...
    public function __call(string $name, array $arguments)
    {
        ...
        trigger_error(sprintf('Call to undefined method: %s::%s()', static::class, $name), E_USER_WARNING);
    }
